...
async function searchArtworks(q, sortBy = 'relevance'){
    ...
    const sql = `
    SELECT a.artworkid AS id,a.name AS title,a.image_url AS imageUrl, art.name AS artistName, m.name AS museumName,p.view_count AS viewCount, p.favourites_count AS favCount, p.average_rating AS avgRating,
    GREATEST(
        similarity(a.name,   $2),
